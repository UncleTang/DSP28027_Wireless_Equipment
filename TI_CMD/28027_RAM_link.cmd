...
*/
//28027的RAM可供用户存放的空间SRAM L0只有4k*16位，空间较少，要注意优化
MEMORY
{
PAGE 0 :
   ...
   PRAML0     : origin = 0x008000, length = 0x000d50     //PRAML0     : origin = 0x008000,  length = 0x00e00
...
PAGE 1 :
   ...
   DRAML0      : origin = 0x008d50, length = 0x0001a0     //DRAML0      : origin = 0x008e00, length = 0x000100
}
...
SECTIONS
{
   ...
   .text            : > PRAML0,    PAGE = 0    //所有可以执行的代码和常量
   .cinit           : > RAMM0,     PAGE = 0    //全局变量和静态变量的C初始化记录
   .pinit           : > RAMM0,     PAGE = 0    //全局构造器（C++）程序列表
   .switch          : > RAMM0,     PAGE = 0    //包含switch声明的列表
   .reset           : > RESET,     PAGE = 0, TYPE = DSECT /* not used, */

   .stack           : > RAMM1,     PAGE = 1    //为系统堆栈保留的空间，用于和函数传递变量或为局部变量分配空间。
   .ebss            : > DRAML0,    PAGE = 1    //为全局变量和局部变量保留的空间
   .econst          : > DRAML0,    PAGE = 1    //包含字符串常量和初始化的全局变量和静态变量（由far const）的初始化和说明   .econst          : > DRAML0,    PAGE = 1
   .esysmem         : > RAMM1,     PAGE = 1    //为动态存储分配保留的空间。如果有宏函数，此空间被宏函数占用，如果没有的话，此空间保留为0

   IQmath           : > DRAML0,    PAGE = 1    //   IQmath           : > PRAML0,    PAGE = 0
