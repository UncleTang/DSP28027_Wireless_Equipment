...
PAGE 0 :
   ...
   PRAML0     : origin = 0x008000, length = 0x001000     //PRAML0     : origin = 0x008000,  length = 0x00e00   d50
...
SECTIONS
{
   ...
   .ebss            : > OTP,    PAGE = 1    //为全局变量和局部变量保留的空间
   .econst          : > OTP,    PAGE = 1    //包含字符串常量和初始化的全局变量和静态变量（由far const）的初始化和说明   .econst          : > DRAML0,    PAGE = 1
   .esysmem         : > RAMM1,     PAGE = 1    //为动态存储分配保留的空间。如果有宏函数，此空间被宏函数占用，如果没有的话，此空间保留为0

   IQmath           : > OTP,    PAGE = 1    //   IQmath           : > PRAML0,    PAGE = 0
